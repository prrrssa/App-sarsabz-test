...
import { Currency, Transaction, Customer, ManagedAccount, CustomerLedgerEntry, User, UserRole, Permission, PersonalExpense, AuditLogEntry, Task, Notification, TaskStatus, TaskPriority, OrnamentalGold, CustomerTier, CustomerTierConfig, AccountAdjustment } from '../types';
import { TOMAN_CURRENCY_CODE, DEFAULT_USERS, PERSONAL_EXPENSE_CATEGORIES, DEFAULT_CUSTOMER_TIER_CONFIG } from '../constants';
...
interface DataContextType {
  ...
  toggleCustomerFavorite: (customerId: string, userId: string) => void;

  managedAccounts: ManagedAccount[];
  addManagedAccount: (account: Omit<ManagedAccount, 'id' | 'balance'> & { initialBalance?: number }, userId: string) => void;
  ...
  addCustomerToCustomerSettlement: (payload: { payerId: string; payeeId: string; currencyId: string; amount: number; description: string; date: string; receiptImage?: string }, userId: string) => boolean;
  ...
  addOrnamentalGoldItem: (itemData: Omit<OrnamentalGold, 'id' | 'addedAt' | 'status' | 'addedByUserId' | 'purchaseWageAmount'>, userId: string) => void;
  ...
  customerTierConfig: CustomerTierConfig;
  updateCustomerTierConfig: (newConfig: CustomerTierConfig, userId: string) => void;
  getCustomerTomanVolume: (customerId: string) => number;
  
  accountAdjustments: AccountAdjustment[];
  adjustAccountBalance: (payload: { accountId: string; adjustmentAmount: number; reason: string }, userId: string) => boolean;
...
export const DataProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [customers, setCustomers] = useLocalStorage<Customer[]>('customers_v4', []);
  ...
  const [users, setUsers] = useLocalStorage<User[]>('users_v2', DEFAULT_USERS); // Ensure users are always available
  ...
  const [customerTierConfig, setCustomerTierConfig] = useLocalStorage<CustomerTierConfig>('customer_tier_config_v1', DEFAULT_CUSTOMER_TIER_CONFIG);
  const [accountAdjustments, setAccountAdjustments] = useLocalStorage<AccountAdjustment[]>('account_adjustments_v1', []);
  ...
  useEffect(() => {
    const seeded = localStorage.getItem('seeded_v20_login_fix'); // Version bump to fix login issues
    if (!seeded) {
      localStorage.clear();
      ...
      const seededCustomers: Customer[] = [
          ...
          { id: 'CUST_03', name: 'رضا محمدی', phoneNumber: '09127778899', nationalId: '1122334455', membershipDate: yesterday.toISOString(), address: 'اصفهان، میدان نقش جهان', notes: 'تماس اولیه از طریق وبسایت.', referralSource: 'وبسایت', isFavorite: false, tier: CustomerTier.BRONZE, points: 5 },
      ];
      ...
      localStorage.setItem('seeded_v20_login_fix', 'true');
    }
  ...
  const addOrnamentalGoldItem = (itemData: Omit<OrnamentalGold, 'id' | 'addedAt' | 'status' | 'addedByUserId' | 'purchaseWageAmount'>, userId: string) => {
  ...
  const deleteOrnamentalGoldItem = (itemId: string, userId: string) => {
      ...
      if (itemToDelete.status === 'sold' && itemToDelete.soldTransactionId) {
          const tx = transactions.find(t => t.id === itemToDelete.soldTransactionId);
          if (tx) {
              // Revert transaction effects without touching the gold item itself
              revertTransactionEffect(tx);
              setCustomerLedger(prev => prev.filter(l => l.transactionId !== tx.id));
              setTransactions(prev => prev.filter(t => t.id !== tx.id));
              addAuditLog(userId, 'delete', 'Transaction', `Deleted transaction #${tx.transactionNumber} as part of deleting sold gold item.`);
          }
      }
  ...
  const toggleCustomerFavorite = useCallback((customerId: string, userId: string) => {
    const customer = getCustomerById(customerId);
    if (!customer) return;
    ...
    const actionText = updatedCustomer.isFavorite ? 'added to favorites' : 'removed from favorites';
    addAuditLog(userId, 'favorite', 'Customer', `Customer '${customer.name}' ${actionText}.`);
  }, [setCustomers, getCustomerById, addAuditLog]);

  // --- Managed Account Functions ---
  const addManagedAccount = (accountData: Omit<ManagedAccount, 'id' | 'balance'> & {initialBalance?: number}, userId: string) => {
  ...
  const addCustomerToCustomerSettlement = useCallback((payload: { payerId: string; payeeId: string; currencyId: string; amount: number; description: string; date: string; receiptImage?: string }, userId: string): boolean => {
    const { payerId, payeeId, currencyId, amount, description, date, receiptImage } = payload;
    const payer = getCustomerById(payerId);
    const payee = getCustomerById(payeeId);
    if (!payer || !payee) {
        alert("مشتری پرداخت کننده یا دریافت کننده یافت نشد.");
        return false;
    }

    const settlementGroupId = `C2C_${Date.now()}`;
    
    const payerEntry: CustomerLedgerEntry = {
        id: `LEDGER_${Date.now()}_P`,
        customerId: payerId,
        date,
        currencyId,
        amount: -amount,
        description: `پرداخت به ${payee.name}: ${description}`,
        settlementGroupId,
        receiptImage,
        createdByUserId: userId,
    ...
    const payeeEntry: CustomerLedgerEntry = {
        id: `LEDGER_${Date.now()}_R`,
        customerId: payeeId,
        date,
        currencyId,
        amount: amount,
        description: `دریافت از ${payer.name}: ${description}`,
        settlementGroupId,
        receiptImage,
        createdByUserId: userId,
        paymentMethod: 'account'
    ...
    setCustomerLedger(prev => [payerEntry, payeeEntry, ...prev].sort((a,b) => new Date(b.date).getTime() - new Date(a.date).getTime()));
    const currency = getCurrencyById(currencyId);
    addAuditLog(userId, 'create', 'CustomerLedger', `Created C2C settlement between ${payer.name} and ${payee.name} for ${formatCurrency(amount, currency?.code)}.`);
    return true;
}, [setCustomerLedger, getCustomerById, addAuditLog, getCurrencyById]);
  ...
  const getCustomerTomanVolume = useCallback((customerId: string): number => {
    const tomanCurrency = currencies.find(c => c.code === TOMAN_CURRENCY_CODE);
    if (!tomanCurrency) return 0;
    ...
    [...transactions].reverse().forEach(tx => {
        if (tx.sourceCurrencyId === tomanCurrency.id && !ratesToToman.has(tx.targetCurrencyId)) {
            ratesToToman.set(tx.targetCurrencyId, tx.sourceAmount / tx.targetAmount);
        } else if (tx.targetCurrencyId === tomanCurrency.id && !ratesToToman.has(tx.sourceCurrencyId)) {
            ratesToToman.set(tx.sourceCurrencyId, tx.targetAmount / tx.sourceAmount);
        }
    });
    ...
    customerTransactions.forEach(tx => {
        const sourceRate = ratesToToman.get(tx.sourceCurrencyId);
        if (sourceRate) {
            totalVolume += tx.sourceAmount * sourceRate;
        }
    });
  ...
  const updateCustomerTierConfig = useCallback((newConfig: CustomerTierConfig, userId: string) => {
    setCustomerTierConfig(newConfig);
    addAuditLog(userId, 'update', 'CustomerTierConfig', 'Updated customer club tier settings.');
  }, [setCustomerTierConfig, addAuditLog]);

  const adjustAccountBalance = useCallback((payload: { accountId: string; adjustmentAmount: number; reason: string }, userId: string): boolean => {
    const { accountId, adjustmentAmount, reason } = payload;
    const account = getManagedAccountById(accountId);
    if (!account) {
        alert("حساب یافت نشد.");
        return false;
    ...
    const adjustmentEntry: AccountAdjustment = {
        id: `ADJ_${Date.now()}`,
        accountId,
        timestamp: new Date().toISOString(),
        userId,
    ...
    setAccountAdjustments(prev => [adjustmentEntry, ...prev].sort((a,b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime()));
    setManagedAccounts(prev => prev.map(acc => acc.id === accountId ? { ...acc, balance: newBalance } : acc));
    const currency = getCurrencyById(account.currencyId);
    addAuditLog(userId, 'update', 'AccountAdjustment', `Adjusted balance for account '${account.name}' by ${formatCurrency(adjustmentAmount, currency?.code)}. Reason: ${reason}`);
    return true;
  }, [getManagedAccountById, setManagedAccounts, setAccountAdjustments, addAuditLog, getCurrencyById]);
  ...
  return (
    <DataContext.Provider value={{
      ...
      customerLedger, addCustomerLedgerEntry, updateCustomerLedgerEntry, deleteCustomerLedgerEntry, addCustomerToCustomerSettlement,
      ...
      customerTierConfig, getCustomerTomanVolume, updateCustomerTierConfig,
      accountAdjustments, adjustAccountBalance
    }}>
      {children}
